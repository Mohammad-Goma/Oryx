...
namespace Oryx
{
    public class Fill_Cbo
    {
        ...
        public void UpdateP1_Parcode(string Finish, TextBox txt_product_parcode)
        {
            ...
            {
                if (con.State == ConnectionState.Open) { con.Close(); }
                string sql = "UPDATE PRODUCTS set QTY = N'" + Finish + "' where PARCODE = N'" + txt_product_parcode.Text.Trim() + "' ";
                con.Open();
                SqlCommand cmd_p1 = new SqlCommand(sql, con);
                cmd_p1.ExecuteNonQuery();
                cmd_p1.Parameters.AddWithValue("@QTY", Finish);
            }
            finally { con.Close(); }
        }
        ...
        public void UpdateP1_Fk(string Finish, TextBox id_fk)
        {
            ...
            {
                if (con.State == ConnectionState.Open) { con.Close(); }
                string sql = "UPDATE PRODUCTS set QTY = N'" + Finish + "' Where ID = N'" + id_fk.Text.Trim() + "' ";
                con.Open();
                SqlCommand cmd_1 = new SqlCommand(sql, con);
                cmd_1.ExecuteNonQuery();
                cmd_1.Parameters.AddWithValue("@QTY", Finish);
            }
            finally { con.Close(); }
        }
